...
class CSPRFBHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 predict_width=False):
        super(CSPRFBHead, self).__init__()
        ...
        self.loss_cls = cls_pos() #分类loss
        self.loss_bbox = reg_pos() #回归
        self.loss_offset = offset_pos() #偏移
        ...
        self.RFB = BasicRFB(feat_channels, feat_channels, stride=1, scale=0.1, visual=1)
    ...
    def _init_layers(self):
        ...
        self.csp_cls = nn.Conv2d(self.feat_channels, self.cls_out_channels, kernel_size = 1)
        self.csp_reg = nn.Conv2d(self.feat_channels, 1, kernel_size = 1)
        self.csp_offset = nn.Conv2d(self.feat_channels, 2, kernel_size = 1)
    ...
    def init_weights(self):
        ...
        xavier_init(self.csp_cls, distribution='normal', bias=bias_cls)
        xavier_init(self.csp_reg, distribution='normal')
        xavier_init(self.csp_offset, distribution='normal')
    ...
    def forward_single(self, x, reg_scale, offset_scale):
        ...
        cls_RFB = self.RFB(cls_feat)
        cls_score = self.csp_cls(cls_RFB)
